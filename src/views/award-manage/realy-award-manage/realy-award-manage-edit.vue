  ...
  <div id="Main">
    ...
    <div class="box">
      <Form ref="form" :model="formData" :label-width="80" :rules="rule">
        <Row>
          <Col span="10">
            <Form-item label="活动时间" required :label-width="100">
              <Row>
                <Col span="11">
          ...
          <Col span="10">
            <Form-item label="活动类型" prop="activityType" :label-width="100">
      ...
      <footer>
        ...
        <Button type="success" @click="submit('form')" style="margin-left:24px">确定</Button>
      </footer>
    </div>
  </div>
</template>
  ...
  data() {
    ...
    return {
      ...
      rule: {
        ...
        activityType: [{ required: true, message: "请选择品牌名称" }],
  ...
  methods: {
    ...
    activityDetail: function(id) {
      ...
      if (id) {
        this.Global.doPost("goodsInfo/getGoodsInfo.json", id, function(res) {
          ...
          that.formData["activityType"] = res.activityType;
    ...
    init: function() {
      ...
      this.Global.doPost(api, data, function(res) {
        that.$router.push("/realyAwardManage");
      });
    },
    goBack: function() {
      this.$router.back();
