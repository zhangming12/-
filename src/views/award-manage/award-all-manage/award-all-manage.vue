...
<template>
  <div id="Main">
      ...
      <Modal
        ...
        <ul id="list">
            <li @click="goEntity">
                <img src="../../../assets/image/s-prize.png" alt="">
                <div>添加实物兑换码</div>
            </li>
            <li @click="goElectron">
                <img src="../../../assets/image/dzq.png" alt="">
                <div>添加电子券</div>
            </li>
        </ul>
      </Modal>
...
import {awardStatus,goodsType} from '@/util/ENUMS.js'

export default {
    name:"award-all-manage-keepAlive",
    data () {
        const that = this;
            ...
            columns1: [
                {
                    ...
                    align: 'center',
                    render: (h,params) => {
                        return goodsType[params.row.type]
                    }
                },
                {
                    title: '折扣名称',
                ...
                },
                {
                    ...
                    render: (h,params) => {      
                        return h('div',{
                            style: {
                                color: params.row.status === 1?'#009847':'#f00'
                            }
                        },awardStatus[params.row.status])
                    }
                },
                {
                    title: '操作',
                                ...
                                on: {
                                    click: () => {
                                        if( params.row.type === 2 ){
                                            this.$router.push({path:'/redAwardManage-edit',query:{type:'edd',id:params.row.id}})
                                        }else if( params.row.type === 1 ){
                                            this.$router.push({path:'/awardManage-edit',query:{type:'edd',id:params.row.id}})
                                        }else{
                                            this.$Message.error('暂无折扣类型')
                                        }
                                    }
                                }
                            }, '修改'),
    ...
    methods: {
        ...
        handleDelete(id){
            var that = this;
            this.Global.doPost('goodsInfo/deleteGoodsInfo.json',{
                id: id,
            },function(){
        ...
        goEntity() {
            this.$router.push({path:'/awardManage-edit',query:{type:'add'}})
        },
        goElectron () {
            this.$router.push({path:'/electronAwardManage-edit',query:{type:'add'}})
        }
