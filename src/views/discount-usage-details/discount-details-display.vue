          ...
          <Form ref="form" :model="formData" :label-width="10">
              <div class="container">
                    ...
                    <div class="btn-left w18">
                        <Form-item  prop="queryStartTime" required>
                            <data-range @dataChange="startTimeChange" placeholder="发放开始时间" hour="00:00" :time="formData.queryStartTime" start></data-range>
                        </Form-item>
                    </div>
                    <div class="btn-left w18">
                        <Form-item  prop="queryEndTime" required>
                            <data-range hour="24:00" placeholder="发放结束时间" @dataChange="endTimeChange" :time="formData.queryEndTime"></data-range>
                        </Form-item>
                    </div>
              ...
              <transition name="fade">
                <div class="container" v-if="showQuery">
                  ...
                  <div class="btn-left w18">
                    <Form-item required>
                        <Input v-model.trim="formData.voucherNo" placeholder="发放流水"></Input>
                    </Form-item>
                    <Form-item  required>
...
export default {
  name: "discount-details-display-keepAlive",
  data() {
    return {
      oneLeverList: [], //一级组织数据
      twoLeverList: [], //二级组织数据
      ...
      columns1: [
        ...
        },
        {
          ...
          tooltip: true,
          render: (h, params) => {
            let arr = ["待提取", "发放中", "已提取", "已退回", "已过期"];
            return h("div", arr[params.row.recStatus]);
          }
        },
  ...
  methods: {
    ...
    init(currentPage, pageSize) {
      ...
      if (
        !this.formData.userId && !this.formData.joinCode &&
      ...
      this.Global.doPost("display/discountDetail.json", data, res => {
        ...
        for (let i = 0, len = res.datalist.length; i < len; i++) {
          ...
          res.datalist[i].recTime = res.datalist[i].recTime ? this.Global.createTime(
    ...
    exportExcel() {
      ...
      this.Global.deleteEmptyProperty(data);
      this.Global.doPost(
        "display/discountDetailExport.json",
        data,
        res => {
          this.myModalisShow = true;
        }
