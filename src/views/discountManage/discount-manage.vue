...
<template>
  <div id="Main">
    ...
    <div class="main-container">
      ...
      <div class="table-box box">
        <div class="contentTop">
          <span class="btn-left">
            此表共包含
            <span class="numColor">{{ pageData.length }}</span>条数据
          </span>
    ...
    <myModal class="myModal" @close="closeModal" width="700" :modal="myModalisShow">
      <div slot="main" class="modal-main">
        ...
        <div class="modal-table">
          <Form ref="form" :model="formDatas" :label-width="88">
            ...
            <Form-item label="折扣单位" required>
              <Input :disabled="type == 'look'" v-model.trim="formDatas.unit" placeholder="折扣单位"/>
            </Form-item>
            <div class="upload-image-box">
              ...
              <div class="upload-box">
                ...
                >
                  <img :src="formDatas.url " alt v-if="formDatas.url ">
            ...
            <Form-item label="使用说明" required>
              <div id="textboxs" style="height:200px;overflow:hidden;border-bottom:1px solid #ccc;"></div>
            </Form-item>
...
export default {
                ...
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.getSummary(params.row.id);
                      // this.editor.txt.html(params.row.summary ? params.row.summary : "");
  ...
  methods: {
    ...
    stopOrStartGoods(data) {
      this.Global.doPost("goodsInfo/deleteGoodsInfo.json", data, res => {
    ...
    getSummary(id) {
      this.Global.doPost("goodsInfo/getGoodsInfo.json", id, res => {
        this.editor.txt.html(res.summary ? res.summary : "");
      });
    },
    saveNewPresent() {
      ...
      if (!this.formDatas.type && this.formDatas.type != "0") {
        this.$Message.error("请选择折扣分类");
        return false;
      }
      ...
      if (!this.formDatas.unit) {
        this.$Message.error("折扣单位不能为空");
        return false;
      }
      if (!this.formDatas.url) {
        this.$Message.error("请上传折扣图片");
        return false;
      }
      if (!this.editor.txt.text()) {
        this.$Message.error("使用说明不能为空");
        return false;
      }
      let data = this.Global.JsonChange(this.formDatas);
      this.Global.deleteEmptyProperty(data);
      data["summary"] = this.editor.txt.html();
      if (this.type == "modify") {
        data["id"] = this.id;
        this.Global.doPost("goodsInfo/modiGoodsInfo.json", data, res => {
          let str = "修改成功";
        ...
        this.Global.doPost("goodsInfo/doSave.json", data, res => {
          let str = "保存成功";
          this.$Message.success(str);
          this.myModalisShow = false;
          this.init();
    ...
    UploadLogoUrl(response, file, fileList) {
      ...
      this.$set(this.formDatas, "url", this.formDatas.url);
    },
    closeModal() {
      this.myModalisShow = false;
    },
    ...
    init(currentPage, pageSize) {
      ...
      this.Global.deleteEmptyProperty(data);
      this.Global.doPost("goodsInfo/queryWithPage.json", data, res => {
        //   console.log(res);
