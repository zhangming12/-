              ...
              <div class="container">
                    ...
                    <div class="btn-left w18">
                        <Form-item  prop="queryStartTime" required>
                            <data-range @dataChange="startTimeChange" placeholder="核销查询开始时间" hour="00:00" :time="formData.queryStartTime" start></data-range>
                        </Form-item>
                    </div>
                    <div class="btn-left w18">
                        <Form-item  prop="queryEndTime" required>
                            <data-range hour="24:00" placeholder="核销查询结束时间" @dataChange="endTimeChange" :time="formData.queryEndTime"></data-range>
                        </Form-item>
                    </div>
                    <div class="btn-left w18">
                      <Form-item required>
...
export default {
  name: "electronic-voucher-write-off-record-keepAlive",
  data() {
    return {
      historyShow: false,
      timeModalShow: false,
      ...
      columns1: [
        ...
        },
        {
          ...
          width: 150,
          render: (h, params) => {
            return h("div", this.Global.createTime(params.row.verifyTime));
          }
        },
        {
          title: "经销商",
        ...
        },
        {
          ...
          render: (h, parmas) => {
            return h(
              "div",
              `${parmas.row.sumWinAmount}${parmas.row.unit}/${
                parmas.row.sumQuantity
              }张`
            );
          }
        },
                        ...
                        this.$Modal.confirm({
                          ...
                          onOk: () => {
                            let url = this.Global.getExportUrl(
                              "ecash/cancelAfterVerificationRecordExport.json",
  ...
  methods: {
    ...
    init(currentPage, pageSize) {
      ...
      this.Global.doPost(
        "ecash/queryCancelAfterVerificationRecord.json",
        data,
        res => {
    ...
    exportExcel() {
      ...
      this.Global.deleteEmptyProperty(data);
      var url = this.Global.getExportUrl("ecash/recordExport.json", data);
      window.open(url);
    },
    changeValue(value) {
      this.distributorList = [];
