...
<script>
import { queryLoongActivityList } from "@/api/landLoong-activity-manage/landLoong-activity-manage.js";
...
export default {
  name:"activity-partake-record-three-keepAlive",
  ...
  methods: {
    ...
    init: function(currentPage, pageSize) {
      ...
      data["loongBizType"] = "three";
      delete data.queryTime;

      queryLoongActivityList(data).then(res => {
        if (res && res.status === 1) {
    ...
    exportExcel: function() {
      ...
      var url = this.Global.getExportUrl(
        "displayLoongLog/loongActivityInfoExport.json",
        data
      );
      window.open(url);
    }
