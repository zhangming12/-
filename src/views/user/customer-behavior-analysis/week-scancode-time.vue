...
<template>
  <div id="Main">
      <div class="box">
            <div id="dayScanCodeTime" :style="{height: '350px'}"></div>
      </div>
  </div>
</template>
...
export default {
    props:{
        weekScanCodeTimeDataList:Array
    },
    watch:{
        weekScanCodeTimeDataList(val){
            if(val){
                console.log(val)
                this.datalist=val;
                this.drawLine(val)
    ...
    methods: {
        drawLine(datalist){
            ...
            let myChart = echarts.init(document.getElementById('dayScanCodeTime'))
            // 绘制图表
            myChart.title = '单轴散点图';
            ...
            echarts.util.each(days, function (day, idx) {
                option.title.push({
                    textBaseline: 'middle',
                    textStyle:{
                        fontStyle:'normal',//默认normal，有italic(斜体),oblique(斜体)
