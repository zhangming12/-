  ...
  name:"activity-partake-summary-keepAlive",
    ...
    methods: {
                    ...
                    for (var j = 0; j < datalist.length; j++) {
                        for (var n = 0; n < keyList.length; n++) {
                            if( datalist[j].weekData ){
                                //datalist[j][`${this.keyList[n].weeks}uploadCount`] = this.keyList[n].uploadCount;
                                datalist[j][  `${keyList[n].weeks}uploadCount`] = datalist[j].weekData[n]['uploadCount'];
                                datalist[j][`${keyList[n].weeks}uploadRate`] = (datalist[j].weekData[n]['uploadRate']*100).toFixed(2)+'%';                                                         
                                //res.datalist[j][that.keyList[n]] = res.datalist[j].countWinMap[that.keyList[n]]
                                //console.log(this.keyList[n].weeks  ,  datalist[j].weekData[n]['uploadCount'] , datalist[j].weekData[n]['uploadRate'])
                                //console.log(`${keyList[n].weeks}uploadCount`,`${keyList[n].weeks}uploadRate`)
        ...
        exportExcel: function() {
            ...
            this.Global.deleteEmptyProperty(data);
            data['brandId'] = 8;            
            var url = this.Global.getExportUrl('report/loongTrackStatisticsForWeekExport.json', data);
            window.open(url);
        }
    }
}
