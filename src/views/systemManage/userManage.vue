...
export default {
  ...
  data() {
    ...
    const validateNewPwd = (rule, value, callback) => {
      ...
      if (value == "") {
        ...
        if (value != this.editData.loginPwd) {
          // 判断两次密码不一致
    ...
    return {
      ...
      rule: {
        ...
        loginAccount: [{ required: true, message: "请输入登陆账号" }],
        userName: [{ required: true, message: "请输入用户姓名" }],
        authCode: [{ required: true, message: "请输入验证码" }],
        loginPwd: [{ validator: validatePwd, required: true }],
        loginNewPwd: [{ validator: validateNewPwd, required: true }]
      },
      pageNum: 0,
      pageSize: 10,
      reviseStatus: false,
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.userBlack(params.row.userId, handle.status);
                        }
                      });
                    }
                  }
  ...
  methods: {
    ...
    init() {
      ...
      this.Global.doPost("admin/doQueryWithPage.json", data, (res) =>{
        this.pageNum = res.items;
        this.pageData = res.datalist;
      });
    },
    userBlack(userId, status) {
      this.Global.doPost(
    ...
    showMemo() {
      ...
      if (this.reviseStatus) {
        this.Global.doPost("admin/doGet.json", this.selectUserId, (
          res
        ) =>{
          this.reviseData["userName"] = res.userName;
          this.reviseData["phone"] = res.phone;
          this.reviseData["loginAccount"] = res.loginAccount;
        });
      }
    },
    ok() {
