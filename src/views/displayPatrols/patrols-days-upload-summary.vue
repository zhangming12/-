                ...
                <Form ref="form" :model="formData" :label-width="10">
                    ...
                    <transition name="fade">
                        <div class="container" v-if="showQuery">
                            <div class="btn-left w18">
                                <Form-item >
                                    <DatePicker type="daterange" v-model="formData.queryTime" show-week-numbers  placeholder="领取日期" style="display:block;"></DatePicker>
                                </Form-item>
...
export default {
  name: "patrols-days-upload-summary-keepAlive",
  ...
  methods: {
    ...
    init() {
      ...
      if(this.formData.queryTime.length && this.formData.queryTime[0]){
        data['queryStartTime'] = this.Global.createTime(this.formData.queryTime[0])
        data['queryEndTime'] = this.Global.createTime(this.formData.queryTime[1])

      }
      delete data['queryTime'];
      console.log(data)
      this.Global.doPost("patrol/queryUploadInfoByDate.json", data, res => {
