...
export default {
  name: "patrols-weeks-upload-summary-keepAlive",
                    ...
                    on: {
                      click: () => {
                        if (params.row.flag) {
                          ...
                          //第一次请求
                          this.Global.doPost(
                            "patrol/getTotalInfo.json",
                            {
                              merchantId: params.row.merchantId,
                              presentId: params.row.presentId,
                              period:params.row.period,
                              queryStartTime: this.Global.createTime(
                                params.row.startTime
                              ),
                              queryEndTime: this.Global.createTime(
                                params.row.endTime
                              )
                            },
                            res => {
                              ...
                              try {
                                params.row.staffNum = res.staffNum;
                                params.row.actNum = res.actNum;
                                params.row.storeNums = res.storeNum;
                                params.row.estimateCost = res.estimateCost;
                                params.row.actualCostBig = res.actualCostBig;
                                params.row.merTotalCost = res.merTotalCost;
                                params.row.merGetCost = res.merGetCost;
                                params.row.merWaitCost = res.merWaitCost;
                                params.row.staffTotalCost = res.staffTotalCost;
                                params.row.staffGetCost = res.staffGetCost;
                                params.row.staffWaitCost = res.staffWaitCost;
                              } catch (error) {
                                console.error(error);
                              }
  ...
  methods: {
    ...
    init() {
      ...
      this.pageData = [];
      this.Global.doPost(
        "patrol/queryUploadTotalInfoByWeek.json",
        data,
        res => {
