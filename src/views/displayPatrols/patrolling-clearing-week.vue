        ...
        <Row>
            ...
            <Form-item label="时间:" prop="weeks">
              <Select v-model="formData.weeks" clearable @on-change="changeTime" v-if="showTime">
                <Option
                  ...
                  :value="key"
                  :key="index"
                >{{ key.split('_')[0] }}年 第{{ key.split("_")[1] }}周 活动日期：{{ item[0] | formatDate}}-{{ item[1] | formatDate}}</Option>
              </Select>
            </Form-item>
          </Col>
          <Col span="7">
          ...
          <Col span="7">
            ...
            <Form-item label="门店ID:">
              <Input v-model="formData.storeId" placeholder="请输入门店ID"></Input>
            </Form-item>
          </Col>
...
export default {
  ...
  created() {
    this.Global.doPost(
      ...
      { date: 15, activityType: 6, scope: "a", liquidatePeriod: "W" },
      res => {
        this.brandList = [];
  ...
  methods: {
    ...
    init() {
      ...
      data["periodMonth"] = parseInt(data["weeks"].split("_")[1]);
    ...
    startClear(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          data["activityType"] = 6;
          data["weeks"] = parseInt(data["weeks"].split("_")[1]);
