...
export default {
  ...
  methods: {
    ...
    activityDetail(queryParams) {
      this.Global.doPost("patrol/getDisplayYxtgInfo.json", queryParams, res => {
        this.formData = res;
        ...
        if (res.radioList && res.radioList.length) {
          for (const key in res.radioList) {
            if (res.radioList[key].locate) {
              locateList.push(res.radioList[key].locate.split(",").reverse());
            }
          }
        }
        let storeLocate = [];
    ...
    handleOnelevelBack() {
      //一审返回
      this.$router.push({ path: "/dealerDisplayAuditPatrols" });
    },
    handleTwolevelBack() {
      //二审返回
      this.$router.push({ path: "/dealerDisplayFinalAuditPatrols" });
    },
    handleMounitorBack() {
      //二审返回
      this.$router.push({ path: "/patrolsUploadMonitor" });
    },
    handleLookMore() {
