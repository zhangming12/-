...
<template>
  <div id="Main">
      <h2 class="Title">河北金银铜活动上传情况统计</h2>
      <div class="box">
...
export default {
  name:"hbGoldActivityUpload-keepAlive",
  ...
  methods: {
    ...
    init: function(currentPage, pageSize) {
      ...
      this.Global.doPost("report/displayUploadCountReport.json", data, res => {
        ...
        if ( res && res.length > 0) {
            for (let j = 0; j < res[0].perList.length; j++) {
                let arr = [
                    {
                      title: `周期${res[0].perList[j].period}未上传数`,
                      key: `${res[0].perList[j].period + 'a'}`,
                      align: "center",
                      minWidth: 120
                    },
                    {
                      title: `周期${res[0].perList[j].period}合格数`,
                      key: `${res[0].perList[j].period + 'b'}`,
                      align: "center",
                      minWidth: 120
                    },
                    {
                      title: `周期${res[0].perList[j].period}不合格数`,
                      key: `${res[0].perList[j].period + 'c'}`,
                      align: "center",
                      minWidth: 120
                    },
                ]; 
            ...
            for (let index = 0; index < res.length; index++) {
                for (let j = 0; j < res[index].perList.length; j++) {
                  res[index][`${res[index].perList[j].period + 'a'}`] = res[index].perList[j].unUploadCount;
                  res[index][`${res[index].perList[j].period + 'b'}`] = res[index].perList[j].passCount;
                  res[index][`${res[index].perList[j].period + 'c'}`] = res[index].perList[j].notPassCount;
                }
            }
        }
        this.pageData = res || [];
    ...
    exportExcel: function() {
      ...
      var url = this.Global.getExportUrl(
        "report/displayUploadCountReportExport.json",
        data
      );
      window.open(url);
    },
