        ...
        if (res && res.length > 0) {
          ...
          for (let index = 0; index < res.length; index++) {
            for (let j = 0; j < res[index].perList.length; j++) {
              res[index][`${res[index].perList[j].period + "a"}`] =
                res[index].perList[j].unUploadCount;
              res[index][`${res[index].perList[j].period + "b"}`] =
                res[index].perList[j].passCount;
              res[index][`${res[index].perList[j].period + "c"}`] =
                res[index].perList[j].notPassCount;
            }
          }
        }
        this.pageData = res || [];
