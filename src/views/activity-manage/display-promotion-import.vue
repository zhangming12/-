...
<template>
	<div id="Main">
        ...
        <transition name="fade">
              ...
              <Form-item label="状态:">
                  <Select v-model="formData.showStatus" placeholder="请选择" clearable>
                      <Option :value="item.value" v-for="(item,index) in statuList" :key="index">{{ item.text }}</Option>
                  </Select>
              </Form-item>
						</Col>
						<Col span='14'>
      ...
      <div class='contentTop'>
          ...
          <Button @click="deleteShow = true" style="float:right;" icon="android-add" type="error">停止</Button>
    ...
    <div style="margin: 10px;overflow: hidden">
      <div style="float: right;">
        ...
        <Modal
          v-model="importShow">
          <h2 style="text-align:center;margin-bottom:10px;">陈列推广名单导入</h2>
          <i-Form>
        ...
        <Modal
          v-model="deleteShow">
          <h2 style="text-align:center;margin-bottom:10px;">陈列推广名单导入删除</h2>
          <i-Form>
            <Form-item label='品牌名称:' prop='brandId' :label-width='72'>
          ...
          <div slot="footer" style="text-align:center;">
            <i-button type="text" @click="cancel">取消</i-button>
            <i-button type="success" @click="deleteUploadExcel">确定</i-button>
          </div>
        </Modal>
        <!-- //修改 -->
        <Modal
          v-model="modifyShow">
          <h2 style="text-align:center;margin-bottom:15px;">修改活动</h2>
          <i-Form :label-width="88">
            <Form-item label="活动时间:" required>
                <Row>
                    ...
                    <Col span="8">
                      <Form-item>
                        <Date-picker type="datetime" placeholder="选择日期" v-model="modifyData.endTime" @on-change='dataChange'></Date-picker>
                      </Form-item>
                    </Col>
                </Row>
            </Form-item>
...
export default {
  name: "display-promotion-import-keepAlive",
  data() {
    return {
      importShow: false,
      showQuery: false,
      ...
      columns1: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              this.formatYearMonth(params.row.showStartTime) +
                "至" +
                this.formatYearMonth(params.row.showEndTime)
            );
          },
          ...
          //   return h(
          //     "div",
          //     this.formatYearMonth(params.row.activityStartTime) +
          //       "至" +
          //       this.formatYearMonth(params.row.activityEndTime)
          //   );
          // }
        },
        {
        ...
        },
        {
          ...
          render: (h, params) => {
            let str = "";
            if (params.row.slackAmount) {
              str = params.row.slackAmount + params.row.unit;
            }
                  ...
                  on: {
                    click: () => {
                      this.$Modal.confirm({
                        ...
                        onOk: () => {
                          this.Global.doPost(
                            "displayYxtg/updateActivityCollectStatus.json",
                            {
                              // brandId: params.row.brandId,
                              brandId: this.formData.brandId,
                              status: 0,
                              joinCode: params.row.joinCode,
                              presentCode: params.row.presentCode
                            },
                            res => {
  ...
  methods: {
    ...
    dataChange(val) {
      this.modifyData.endTime = val.slice(0, 11) + "23:59:59";
    },
    modify(val) {
      //修改
    ...
    init() {
      ...
      data["showStatus"] = Number(data["showStatus"]);
      this.Global.doPost(
