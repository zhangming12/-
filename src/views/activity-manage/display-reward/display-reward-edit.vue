            ...
            <Row>
              ...
              <Col span="24">
                <Row>
                  ...
                  <Col span="4">
                    <Form-item :label-width="10" required>
                      <Checkbox :disabled="type == 'look'" v-model="formData.goodsOpen">启用进货标准</Checkbox>
                    </Form-item>
                  </Col>
                  <Col span="4">
                    <Form-item :label-width="10" required>
                      <Checkbox :disabled="type == 'look'" v-model="formData.showDiscount">启用活动上显示折扣</Checkbox>
                    </Form-item>
                  </Col>
                </Row>
              </Col>
              <Col span="24">
                <Form-item label="周期管理:" required>
                  <Row>
                    ...
                    <Col span="4">
                      <span v-if="type == 'look'">{{ formData.uploadChannel | filterUploadChannel}}</span>
                      <Select v-else v-model="formData.uploadChannel" placeholder="选择上传渠道">
                        <Option
                          :value="item.id"
                          v-for="(item,index) in uploadChannelList"
                          :key="item.id"
                        >{{ item.name }}</Option>
                      </Select>
                    </Col>
                ...
                <Col span="5">
                  <div class="changeBtn">
                    <Upload
                      action="https://hbrand.oss-cn-hangzhou.aliyuncs.com"
                      :data="upData"
                      :before-upload="beforeUploadImage2"
        ...
        <div class="activityBox">
          ...
          <div class="activityContent" v-if="displayActivityList.length">
            ...
            >
              ...
              <Col span="2">
                <div class="ContentMain">
                  <select v-if="item.isModify" v-model="item.isFirstWeek" class="skuandmust">
                    <option :value="1">是</option>
                    <option :value="0">否</option>
                  </select>
                  <span v-else>{{item.isFirstWeek | weekFilter}}</span>
                </div>
              </Col>
              <Col span="2">
                <div class="ContentMain">
...
export default {
  ...
  methods: {
    ...
    saveGroup() {
      ...
      if (!this.formData.uploadChannel) {
        this.$Message.info("请选择上传渠道");
        return false;
      }
      ...
      data["goodsOpen"] = this.formData.goodsOpen ? 1 : 0;
      data["uploadChannel"] = this.formData.uploadChannel;

      data["liquidatePeriod"] = this.formData.liquidatePeriod;

      data["showDiscount"] = this.formData.showDiscount ? 1 : 0;
    ...
    saveActivity(index) {
      ...
      data["isFirstWeek"] = this.displayActivityList[index].isFirstWeek; //固定首周
    ...
    checkedOne(id) {
      for (let i = 0; i < this.areaData.length; i++) {
        if (this.areaData[i].value == id) {
          this.areaData[i].checked = true;
          return;
        }
        if (this.areaData[i].children) {
          for (let j = 0; j < this.areaData[i].children.length; j++) {
            if (this.areaData[i].children[j].value == id) {
              this.areaData[i].children[j].checked = true;
              return;
            }
          }
        }
    ...
    beforeUpload(res) {
      this.image1TimeStr = Date.now();
      this.upData["key"] = "ecuda/image/" + this.image1TimeStr + res.name;
    },
    beforeUploadImage(res) {
      this.image2TimeStr = Date.now();
      this.upData["key"] = "ecuda/image/" + this.image2TimeStr + res.name;
      return this.checkImageWH(res, 700, 240);
    },
    beforeUploadImage2(res) {
      this.image3TimeStr = Date.now();
      this.upData["key"] = "ecuda/image/" + this.image3TimeStr + res.name;
      return this.checkImageWH(res, 700, 240);
    },
      ...
      if (id && brandId) {
        this.Global.doPost(
          ...
          res => {
            ...
            this.formData["goodsOpen"] = res.goodsOpen ? true : false;
            this.formData["uploadChannel"] = res.uploadChannel;
            ...
            this.formData["showDiscount"] = res.showDiscount ? true : false;
