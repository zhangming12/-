...
<template>
  <div id="Main">
              ...
              <template v-if="item && item.checkStatus != -1">
                <div class="video-box">
                  <div class="video">
                    ...
                    <div class="video-main" v-if="item.fileType == 'image'">
                      <imageLook
                        :key="item.firstImageList[0]"
                        :position="getPosition(index)"
                        v-if="item.firstImageList.length"
                        :imageList="item.firstImageList"
                      />
                  ...
                  <Row style="width:100%;">
                    <Col span="8">
                      <div class="item-box">
                        <p>
                          <span>提交时间：</span>
                          <span>{{ item.submitTime | formatYearMonth}}</span>
                        </p>
                      </div>
                      <div class="item-box">
                        <p :title="item.activityTag">
                          <span>活动分组：</span>
                    ...
                    <Col span="8">
                      ...
                      <div class="item-box">
                        <p>
                          <span>AI审核状态：</span>
                          <span>{{ item.aiStatus | aiFilter }}</span>
                        </p>
                      </div>
                    </Col>
                    <Col span="8">
                      ...
                      <div class="item-box">
                        <p :title="item.storeName">
                          <span>门店名称：</span>
                          <span>{{ item.storeName }}</span>
                        </p>
                      </div>
                      <div class="item-box">
                        <p :title="item.storeRange > 200 ? '位置偏移' :''">
                          <span>预警：</span>
                          <span
                    ...
                    <Col span="23">
                      <div style="border-top:1px solid #666;border-bottom:1px solid #666;">
                        <Row>
                          ...
                          <Col span="4">
                            <div class="item-box">
                              <p :title="item.checkFirstOneUserId">
                                <span>{{item.checkFirstOneUserId}}</span>
                              </p>
                            </div>
                          </Col>
                          <Col span="3">
                            <div class="item-box">
                              <span>{{item.checkFirstOneStatus | statusFilter}}</span>
                            </div>
                          </Col>
                          <Col span="15">
                            <div class="item-box">
                              <p :title="item.checkFirstOneMessage">
                                <span>审核意见：</span>
                                <span>{{ item.checkFirstOneMessage | messageFilter}}</span>
                              </p>
                            </div>
                          </Col>
                          <Col span="24" v-if="item.checkProjectOne">
                            <Row>
                              <Col span="5" v-for="(v,n,i) in item.checkProjectOne" :key="i">
                                <div class="item-box">
                                  <Checkbox disabled :value="v == 1 ?true : false">{{n}}</Checkbox>
                                </div>
                              </Col>
                              <Col span="4" v-if="item.skuNecessaryOne != item.skuNecessaryTwo">
                                <div class="item-box">
                                  <p title="SKU明细差异">SKU明细差异</p>
                                </div>
                              </Col>
                            </Row>
                          </Col>
                        ...
                        <Row>
                          ...
                          <Col span="4">
                            <div class="item-box">
                              <p :title="item.checkFirstTwoUserId">
                                <span>{{item.checkFirstTwoUserId}}</span>
                              </p>
                            </div>
                          </Col>
                          <Col span="3">
                            <div class="item-box">
                              <span>{{item.checkFirstTwoStatus | statusFilter}}</span>
                            </div>
                          </Col>
                          <Col span="15">
                            <div class="item-box">
                              <p :title="item.checkFirstTwoMessage">
                                <span>审核意见：</span>
                                <span>{{ item.checkFirstTwoMessage | messageFilter}}</span>
                              </p>
                            </div>
                          </Col>
                          <Col span="24" v-if="item.checkProjectTwo">
                            <Row>
                              <Col span="5" v-for="(v,n,i) in item.checkProjectTwo" :key="i">
                                <div class="item-box">
                                  <p :title="n">
                                    <Checkbox disabled :value="v == 1 ?true : false">{{n}}</Checkbox>
                                  </p>
                      ...
                      <div class="item-box" v-if="item.isOpenCheck == 1">
                        <check-look
                          ...
                          @getSkuNecessary="getSkuNecessary"
                          :ind="index"
                          :showAudit="item.presentSkuVO.isSkuNecessary == 1"
                          :allData="item"
                        ></check-look>
                      </div>
                      <div class="item-box">
                        ...
                        <RadioGroup v-model="item.status" @on-change="radioChange">
                          <Radio :label="index + '-2001'">通过</Radio>
                          <Radio :label="index + '-2'">不通过</Radio>
                          <Radio v-if="item.isBack == 0" :label="index + '-3'">退回</Radio>
                        </RadioGroup>
                        <div class="tooltip" style="display:inline-block;">
                          <my-tooltip
                            :auditPolicy="item.auditPolicy"
    ...
    >
      ...
      <div>
        <Form :model="riskData" :label-width="90">
          <FormItem label="客户编号:">{{ joinCodeList.join() }}</FormItem>
          <FormItem label="风险类型:">
            <Select v-model="riskData.riskType" placeholder="风险类型">
              <Option value="no">无</Option>
              <Option value="V">视频异常</Option>
            </Select>
          </FormItem>
          <FormItem label="风险描述:">
            <Input v-model.trim="riskData.riskDes" placeholder="风险描述"></Input>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
        <Button type="success" @click="riskIsShow = false;">取消</Button>
        <Button type="warning" @click="riskSure">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getSkuNecessary(obj) {
      let { index, skuNecessary } = obj;
      this.storeGoodsList[index].skuNecessary = skuNecessary;
    },
    getRadioData(v) {
      let { index, data, len } = v;
      this.storeGoodsList[index].checkProject = data;
    ...
    riskSure() {
      ...
      let dataList = [];
      if (this.riskData.riskType != "no" && !this.riskData.riskDes) {
        this.$Message.error("风险描述不能为空");
        return false;
      }
      let { riskType, riskDes } = this.riskData;
      if (riskType == "no") {
        riskType = "";
      }
      for (let i = 0; i < len; i++) {
        let obj = {
          activityId: this.storeGoodsList[this.social[i]].activityId,
          storeId: this.storeGoodsList[this.social[i]].storeId,
          period: this.storeGoodsList[this.social[i]].period,
          riskSource: "H",
          riskDes,
          riskType
        };
      ...
      this.Global.doPost(
        "displayYxtg/addVideoRiskForList.json",
        dataList,
        res => {
          this.$Message.success("操作成功");
          this.riskIsShow = false;
          this.init();
        }
      );
    },
    riskShow() {
      ...
      let len = this.social.length;
      for (let i = 0; i < len; i++) {
        this.joinCodeList.push(this.storeGoodsList[this.social[i]].joinCode);
      }
    ...
    radioChange(val) {
      let arr = val.split("-");
      this.formItem.checkStatus = arr[1];
      this.index = arr[0];
      this.saveData.brandId = this.storeGoodsList[arr[0]].brandId;
      this.saveData.groupId = this.storeGoodsList[arr[0]].groupId;
      this.formItem.isBack = this.storeGoodsList[arr[0]].isBack;
    ...
    save(item, index) {
      ...
      if (item.isOpenCheck == 1 || data) {
        let entries = Object.values(item.checkProject);
        let len = 0;
        for (let i = 0; i < entries.length; i++) {
          len += entries[i];
        }
        if (!len && statusC == "2001") {
          this.$Message.info("只有考核项全部通过才能选择通过");
          return false;
        }
        if (len == item.len) {
          if (statusC != 2001) {
            this.$Message.info("考核项全部通过只能选择通过");
            return false;
          }
        } else {
          if (statusC == 2001) {
        ...
        params["checkProject"] = item.checkProject;
      }
    ...
    init() {
      ...
      data["level"] = 1;
      this.Global.doPost(
        "displayYxtg/queryDifferentVideoCheckStatus.json",
        data,
        res => {
    ...
    handleLookDetail(val, flag) {
      ...
      queryParams["displayActCategory"] = "singleTwo";
    ...
    handleBatchSure() {
      ...
      let dataList = [];
      for (let i = 0; i < len; i++) {
        let arr = this.social[i];
        let statusC = this.storeGoodsList[this.social[i]].status;
        if (!statusC) {
        ...
        statusC = statusC.split("-")[1];
        if (statusC == 2 || statusC == 3) {
          if (!this.storeGoodsList[this.social[i]].checkMessage) {
            this.$Message.error("请输入审核意见");
            return false;
          }
        }
        let obj = {
          id: this.storeGoodsList[this.social[i]].id,
          brandId: this.storeGoodsList[this.social[i]].brandId,
          activityId: this.storeGoodsList[this.social[i]].activityId,
          groupId: this.storeGoodsList[this.social[i]].groupId,
          memo: this.storeGoodsList[this.social[i]].memo,
          checkMessage: this.storeGoodsList[this.social[i]].checkMessage,
          checkStatus: statusC
        };
        this.Global.deleteEmptyProperty(obj);
        dataList.push(obj);
      ...
      // return /
      this.Global.doPost("displayYxtg/checkOne.json", dataList, res => {
