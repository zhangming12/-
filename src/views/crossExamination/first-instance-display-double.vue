            ...
            <div class="card-top">
              <div class="card-left">
                <template v-if="item.store && item.store.checkStatus != -1">
                  ...
                  <div class="video-box">
                    <div class="video">
                      <!-- v-if="item.store.fileType == 'radio'" -->
                  ...
                  <div class="content-box">
                    <Row style="width:100%;">
                      <Col span="8">
                        ...
                        <div class="item-box">
                          <span>渠道：</span>
                          <span>{{ item.store.bapChannel }}</span>
                        </div>
                      </Col>
                      <Col span="8">
                        <div class="item-box">
                      ...
                      <Col span="23">
                        <div class="item-box">
                          ...
                          <RadioGroup v-model="item['store'].status" @on-change="radioChange">
                            <Radio :disabled="item.store.userId" :label="index + '-store-1001'">通过</Radio>
                            <Radio :disabled="item.store.userId" :label="index + '-store-2'">不通过</Radio>
                            <Radio
                              v-if="item['store'].isBack == 0"
                              :disabled="item.store.userId"
                              :label="index + '-store-3'"
                            >退回</Radio>
                          </RadioGroup>
              ...
              <div class="card-right">
                <template v-if="item.worker && item.worker.checkStatus != -1">
                  ...
                  <div class="video-box">
                    <div class="video">
                      <!-- v-if="item.worker.fileType == 'radio'" -->
                  ...
                  <div class="content-box">
                    <Row style="width:100%;">
                      <Col span="8">
                        ...
                        <div class="item-box">
                          <span>渠道：</span>
                          <span>{{ item.worker.bapChannel }}</span>
                        </div>
                      </Col>
                      <Col span="8">
                        <div class="item-box">
                      ...
                      <Col span="23">
                        <div class="item-box">
                          ...
                          <RadioGroup v-model="item['worker'].status" @on-change="radioChange">
                            <Radio :disabled="item.worker.userId" :label="index + '-worker-1001'">通过</Radio>
                            <Radio :disabled="item.worker.userId" :label="index + '-worker-2'">不通过</Radio>
                            <Radio
                              v-if="item['worker'].isBack == 0"
                              :disabled="item.worker.userId"
...
export default {
  name: "first-instance-display-double-keepAlive",
  components: {
    dataRange,
    imageLook,
    myTooltip
  ...
  //   //陈列一审
  //   next(vm => {
  //     vm.displayExamineType = "A";
  //     // vm.storeGoodsList = null;
  //     vm.checkStatus = {
  //       success: "1001",
  //       fail: "1002",
  //       back: "1003"
  ...
  methods: {
    ...
    saveWarning(item, index) {
      ...
      if (item.store) {
        let { storeId, period, activityId, id } = item.store;
        data.push({
          storeId,
          period,
          activityId,
      ...
      if (item.worker) {
        let { storeId, period, activityId, id } = item.worker;
        data.push({
          storeId,
          period,
          activityId,
      ...
      this.Global.doPost("displayYxtg/addVideoRiskForList.json", data, res => {
    ...
    init() {
      ...
      this.Global.doPost("displayYxtg/doubleVideoAudit.json", data, res => {
        ...
        for (let i = 0, len = res.datalist.length; i < len; i++) {
          ...
          let workerUserId =
            ...
            "";
          let storeStatus =
            res.datalist[i].store.checkFirstTwoStatus ||
            res.datalist[i].store.checkFirstOneStatus ||
            "";
          let workerStatus =
            res.datalist[i].worker.checkFirstTwoStatus ||
            res.datalist[i].worker.checkFirstOneStatus ||
            "";

          if (storeStatus && storeTrue) {
          ...
          if (storeUserId && storeTrue) {
            res.datalist[i].store.userId = true;
          }
          if (workerUserId && workerTrue) {
            res.datalist[i].worker.userId = true;
          }
          let storeMessage =
            res.datalist[i].store.checkFirstOneMessage ||
            res.datalist[i].store.checkFirstTwoMessage ||
            "";
          let workerMessage =
            res.datalist[i].worker.checkFirstOneMessage ||
            res.datalist[i].worker.checkFirstTwoMessage ||
            "";

          if (storeMessage && storeTrue) {
            res.datalist[i].store.checkMessage = storeMessage;
          }
          if (workerMessage && workerTrue) {
    ...
    handleLookDetail(val, flag) {
      ...
      queryParams["displayActCategory"] = "doubleOne";
